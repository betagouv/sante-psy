...
async function insertPatientInfoInDb(psy: Psychologist, student?) {
  const theStudent = student || (await create.insertOneStudent());
  const patient = await dbPatients.insert(psy.dossierNumber, theStudent.id);
...
describe('appointmentsController', () => {
  ...
  describe('create appointment', () => {
    ...
    it('should create appointments', async () => {
      ...
      res.body.message.should.equal(
        `La séance du ${formattedDate} a bien été créée, et l'étudiant en a été informé par email.`,
      );
      ...
      expect(appointmentArray[0].psychologistId).to.equal(
        psy.dossierNumber
      );
    });
    ...
    it('should not create appointment if student has 12 already (2 psys)', async () => {
      ...
      // Même étudiant, 2 patients (1 par psy)
      const student = await create.insertOneStudent();
      const dbPatient1 = await dbPatients.insert(psy1.dossierNumber, student.id);
      const dbPatient2 = await dbPatients.insert(psy2.dossierNumber, student.id);
  ...
  describe('get appointments with badges', () => {
    ...
    it('should have appointment with a specific badge for the 1st appointment', async () => {
      const dbPatient1 = await insertPatientInfoInDb(psy);
