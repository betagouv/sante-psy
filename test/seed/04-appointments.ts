...
const getOneAppointmentPerMonth = (patient: {id: string, psychologistId: string}, day: number, deleted = false)
  ...
  .map((i) => create.getOneAppointment(patient.id, patient.psychologistId, i, day, deleted));
  ...
  if (fixedValues) {
    ...
    appointmentList = patientList.flatMap((patient) => {
      ...
      for (let i = 0; i < nbOfAppointments; i++) {
        ...
        result.push(create.getOneAppointment(patient.id, patient.psychologistId, date.getMonth(), date.getDate()));
      }
      return result;
    });
  }
