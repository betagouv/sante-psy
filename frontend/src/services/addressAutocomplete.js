...
// Unique regions list
const regions = [...new Set(Object.values(departementToRegion))];
...
const searchLocalData = (query) => {
    ...
    Object.keys(departementToRegion).forEach(dept => {
        ...
        if (deptCode.includes(normalizedQuery)) {
            results.push({
                value: dept,
                label: dept,
                type: 'departement',
...
const searchAddresses = async (query, limit = 5) => {
    ...
    try {
        ...
        if (query.length <= 3 && localResults.length > 0) {
        ...
        const apiParams = {
            q: query,
            limit: Math.max(1, limit - localResults.length),
            timeout: 5000,
        };
