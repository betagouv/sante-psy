...
async function sendStudentMailTemplate(
  email: string,
  loginUrl: string,
  token: string,
  template: string,
...
async function saveStudentToken(email: string, token: string, expiredAt: string): Promise<void> {
  try {
    const existing = await dbStudentLoginToken.getStudentByEmail(email);
    if (existing) {
    ...
    throw new CustomError('Erreur de sauvegarde du token', 500);
  }
}
...
const signInValidator = [
  check('firstNames').isString().trim().notEmpty()
